...
float ComplexFigure(const sf::Vector2f &point, const sf::Texture &texture)
{
	sf::FloatRect rectangleRect(100.0f, 100.0f, 200.0f,
								150.0f); // Прямоугольник с левым верхним углом в (100, 100) и размерами 200x150
	sf::Vector2f pointToCheck(150.0f, 120.0f); // Точка, для которой вы хотите проверить расстояние до прямоугольника

	const sf::Vector2f circleCenter1{static_cast<float>(texture.getSize().x) / 2,
									 static_cast<float>(texture.getSize().y) / 2 + 100};
	const float circleRadius1 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 6;

	const sf::Vector2f circleCenter2{static_cast<float>(texture.getSize().x) / 2,
									 static_cast<float>(texture.getSize().y) / 2 - 75};
	const float circleRadius2 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 8;

	const sf::Vector2f circleCenter3{static_cast<float>(texture.getSize().x) / 2 - 50,
									 static_cast<float>(texture.getSize().y) / 2 - 170};
	const float circleRadius3 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 25;

	const sf::Vector2f circleCenter4{static_cast<float>(texture.getSize().x) / 2 + 50,
									 static_cast<float>(texture.getSize().y) / 2 - 170};
	const float circleRadius4 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 25;

	const sf::Vector2f circleCenter5{static_cast<float>(texture.getSize().x) / 2 + 130,
	...
	const sf::Vector2f circleCenter6{static_cast<float>(texture.getSize().x) / 2 + 130,
									 static_cast<float>(texture.getSize().y) / 2 + 150};
	const float circleRadius6 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 10;

	const sf::Vector2f circleCenter7{static_cast<float>(texture.getSize().x) / 2 - 130,
									 static_cast<float>(texture.getSize().y) / 2 + 50};
	const float circleRadius7 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 10;

	const sf::Vector2f circleCenter8{static_cast<float>(texture.getSize().x) / 2 - 130,
									 static_cast<float>(texture.getSize().y) / 2 + 150};
	const float circleRadius8 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 10;
	...
	float result3 = Functions::Circle(circleCenter3, circleRadius3, point);
	...
	float result7 = Functions::Circle(circleCenter7, circleRadius7, point);
	float result8 = Functions::Circle(circleCenter8, circleRadius8, point);

	float res1 = Functions::ROr(result5, result6);
	float res2 = Functions::RAnd(result1, res1);
	float res3 = Functions::ROr(result7, result8);
	float res4 = Functions::RAnd(res2, res3);
	float res5 = Functions::RAnd(result2, result3);
	float res6 = Functions::RAnd(res5, result4);
	float res7 = Functions::RAnd(res4, res6);
