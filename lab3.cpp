...
float ComplexFigure(const sf::Vector2f &point, const sf::Texture &texture)
{
	...
	const sf::Vector2f circleCenter1 = originalCenter + sf::Vector2f(0, 100) * scaleFactor;
	const float circleRadius1 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 6 * scaleFactor;

	const sf::Vector2f circleCenter2 = originalCenter + sf::Vector2f(0, -75) * scaleFactor;
	const float circleRadius2 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 8 * scaleFactor;

	const sf::Vector2f circleCenter3 = originalCenter + sf::Vector2f(-50, -170) * scaleFactor;
	const float circleRadius3 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 25 * scaleFactor;

	const sf::Vector2f circleCenter4 = originalCenter + sf::Vector2f(50, -170) * scaleFactor;
	const float circleRadius4 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 25 * scaleFactor;

	const sf::Vector2f circleCenter5 = originalCenter + sf::Vector2f(130, 50) * scaleFactor;
	const float circleRadius5 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 10 * scaleFactor;

	const sf::Vector2f circleCenter6 = originalCenter + sf::Vector2f(130, 150) * scaleFactor;
	const float circleRadius6 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 10 * scaleFactor;

	const sf::Vector2f circleCenter7 = originalCenter + sf::Vector2f(-130, 50) * scaleFactor;
	const float circleRadius7 =
		std::min(static_cast<float>(texture.getSize().x), static_cast<float>(texture.getSize().y)) / 10 * scaleFactor;

	const sf::Vector2f circleCenter8 = originalCenter + sf::Vector2f(-130, 150) * scaleFactor;
